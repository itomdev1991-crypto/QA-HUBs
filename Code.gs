...
const SS_ID = '1x-WEHf1Q1wF_A7E8ngx1Nu7i7oCWZ74Ys3sgqHtW0lk'; // ← ใส่ Spreadsheet ID
...
// XOR cipher + base64 prefix "ENC:" — ต้องตรงกับ ENCRYPT_KEY ใน index.html
// เปลี่ยน key ได้ แต่ต้องอัปเดตทั้ง index.html และ Code.gs พร้อมกัน
// แล้ว re-seed หรือ re-encrypt ข้อมูลเดิมใน Sheet ทั้งหมด

var ENCRYPT_KEY = 'QAH@S3cr3tK3y!2025#XZ'; // ← ต้องตรงกับ index.html

// Decode "ENC:..." → plain string; คืนค่าเดิมถ้าไม่ได้ถูก encrypt
function decField(str) {
  if (!str || typeof str !== 'string') return str;
  if (str.indexOf('ENC:') !== 0) return str; // ยังเป็น plain (legacy / unencrypted row)
  try {
    var b64 = str.slice(4);
    var decoded = Utilities.base64Decode(b64); // returns byte array
    var k = ENCRYPT_KEY;
    var out = '';
    for (var i = 0; i < decoded.length; i++) {
      out += String.fromCharCode(decoded[i] ^ k.charCodeAt(i % k.length));
    }
    return out;
  } catch(e) {
    Logger.log('decField error: ' + e.message + ' val=' + str);
    return str;
  }
}

// Fields ที่เข้ารหัสไว้ (ต้องตรงกับ ENC_FIELDS ใน index.html)
var ENC_FIELDS = { users: ['email','password'], teams: ['inviteCode'] };

function decryptRow(obj, sheetKey) {
  var fields = ENC_FIELDS[sheetKey];
  if (!fields) return obj;
  var copy = {};
  var keys = Object.keys(obj);
  for (var i = 0; i < keys.length; i++) {
    var k = keys[i];
    copy[k] = (fields.indexOf(k) !== -1) ? decField(obj[k]) : obj[k];
  }
  return copy;
}
...
function rowsToObjects(sheet, sheetKey) {
  ...
  // ── Decrypt sensitive fields before returning to browser ─────────
  return mapped.map(function(o) { return decryptRow(o, sheetKey); });
}
